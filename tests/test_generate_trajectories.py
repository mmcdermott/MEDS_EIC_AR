...
def test_generate_trajectories_rolling_runs(
    ...
    The rolling fixture requests ``rolling_generation.max_new_tokens=30`` on the demo model, whose
    ``pretrained_max_seq_len=20``. The single-chunk (non-rolling) fixture, by contrast, caps the new-token
    budget at ``max_seq_len - input_len <= 19`` — strictly less than 30. So any (split, sample, subject)
    whose rolling output has **strictly more** generated rows than its non-rolling counterpart is direct
    evidence that the rolling path emitted output the single-chunk path mathematically cannot. This is the
    actual integration signal: the shape-only assertions below (non-empty parquet files, subjects line up
    across samples) would pass even if the rolling kwarg were silently ignored and the CLI fell through to
    the legacy path. The ``max_rolling_rows > max_single_rows`` check is what proves the plumbing.
    """
    ...
    assert len(rolling_by_split) == 3, "Not all splits have rolling-generated trajectories."
    ...
    # tokens per subject (since the prompt consumes at least one slot). Rolling asked for 30, which the
    # single-chunk path cannot produce. If ``max_rolling_rows`` exceeds the non-rolling ceiling, the
    # rolling loop must have iterated across at least one chunk boundary and written its output through.
    assert max_rolling_rows > max_single_rows, (
        ...
        f"Expected rolling to emit tokens beyond the single-chunk cap with max_new_tokens=30 / "
